...
const StatCard = ({ icon, label, value, sub, color }: {
  ...
  <Card className="group border border-border bg-card rounded-[2.5rem] overflow-hidden hover:shadow-xl transition-all duration-300 shadow-sm">
    <CardContent className="p-8">
      <div className="flex items-start justify-between">
        <div>
          ...
          {sub && <p className="text-[9px] font-bold text-muted-foreground mt-2 uppercase tracking-widest">{sub}</p>}
        </div>
...
const AnalyticsDashboard = () => {
  ...
  if (loading) return (
    <div className="space-y-10">
      <div className="h-10 bg-slate-50 border border-border rounded-2xl w-64 animate-pulse" />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-40 bg-slate-50 border border-border rounded-[2.5rem] animate-pulse" />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-10 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
        <div>
           <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tighter leading-none mb-4">Performance <span className="text-primary">Telemetry</span></h1>
           <p className="text-muted-foreground font-medium text-lg">Real-time engagement metrics and traffic distribution signatures.</p>
        </div>
      </div>

      {/* Stat Grid */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        <StatCard icon={<Eye size={22} className="text-primary" />} label="Surface Scans" value={data.totalViews} color="text-primary" sub="Total Page Views" />
        <StatCard icon={<Zap size={22} className="text-primary" />} label="Action Events" value={data.demoClicks} color="text-primary" sub="Interactive Clicks" />
        ...
        <StatCard icon={<Mail size={22} className="text-primary" />} label="Inbound Leads" value={data.contactSubmissions} color="text-primary" sub="Form Submissions" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        {/* Graph Card */}
        <Card className="lg:col-span-2 border border-border bg-card rounded-[3rem] overflow-hidden shadow-sm">
           <CardHeader className="p-10 pb-4 flex flex-row items-center justify-between">
              <div className="flex items-center gap-3">
                 <div className="w-10 h-10 rounded-2xl bg-primary/5 text-primary flex items-center justify-center border border-primary/15">
                    <TrendingUp size={20} />
                 </div>
                 <div>
                    <CardTitle className="text-xl font-black tracking-tighter uppercase italic">Traffic Intensity</CardTitle>
                    <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mt-1">7-Day Analysis Window</p>
                 </div>
              </div>
        ...
        {/* Top Entities */}
        <Card className="border border-border bg-card rounded-[3rem] overflow-hidden shadow-sm flex flex-col">
           <CardHeader className="p-10 pb-4">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    ...
                    <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mt-1">High Engagement Nodes</p>
                 </div>
              </div>
           </CardHeader>
                    ...
                    {data.topProjects.slice(0, 5).map((project, i) => (
                       <div key={i} className="flex flex-col gap-2 group">
                          <div className="flex items-center justify-between">
                             <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 truncate max-w-[70%] group-hover:text-primary transition-colors italic">{project.title}</span>
                             <span className="text-[10px] font-black text-primary bg-primary/5 px-3 py-1 rounded-full">{project.count} hits</span>
                          </div>
      ...
      <Card className="border border-border bg-card rounded-[3rem] overflow-hidden shadow-sm">
        <CardHeader className="p-10 border-b border-border bg-slate-50/20 flex flex-row items-center justify-between">
           <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-card border border-border text-muted-foreground flex items-center justify-center">
                 <Clock size={20} />
              </div>
              ...
              {data.recentEvents?.length > 0 ? (
                 <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50/50 sticky top-0 z-10">
                       <tr>
                          <th className="p-6 pl-10 text-[9px] font-black uppercase tracking-widest text-muted-foreground">Sector Code</th>
                          <th className="p-6 text-[9px] font-black uppercase tracking-widest text-muted-foreground">Node Identifier</th>
                          <th className="p-6 text-[9px] font-black uppercase tracking-widest text-muted-foreground">Time Signature</th>
                          <th className="p-6 pr-10 text-[9px] font-black uppercase tracking-widest text-muted-foreground text-right">Status</th>
                       </tr>
                    </thead>
                          ...
                          <tr key={i} className="hover:bg-slate-50/50 transition-colors group">
                             <td className="p-6 pl-10">
                                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                                  e.type === 'page_view' ? 'bg-primary/5 text-primary' :
                                  e.type === 'demo_click' ? 'bg-indigo-50 text-indigo-600' :
                                  'bg-slate-50 text-muted-foreground'
                                }`}>{e.type.replace('_', ' ')}</span>
                             </td>
                             <td className="p-6">
                                <span className="text-[10px] font-black text-foreground uppercase italic group-hover:text-primary transition-colors">
