  ...
  return (
    <div className="space-y-10 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
        <div>
          <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tighter leading-none mb-4">Recognition <span className="text-primary">Node</span></h1>
              ...
              <Card className="border border-border shadow-2xl rounded-[3rem] overflow-hidden bg-card">
                <CardHeader className="p-10 border-b border-border flex flex-row items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs font-black uppercase tracking-widest text-muted-foreground mt-1">Data Entry Module V1.0</p>
                  </div>
                  <Button variant="ghost" onClick={() => setShowForm(false)} className="rounded-full w-12 h-12 p-0 bg-slate-50 text-muted-foreground hover:text-foreground"><X size={20} /></Button>
                </CardHeader>
                <CardContent className="p-10 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                  <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="md:col-span-2 space-y-3">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Achievement Label</Label>
                    ...
                    <div className="space-y-3">
                      ...
                      <select name="category" value={form.category} onChange={handleChange} className="w-full h-14 bg-slate-50/50 border border-border rounded-2xl px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-primary transition-all">
                    ...
                    <div className="space-y-3">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Node Type</Label>
                      <select name="type" value={form.type} onChange={handleChange} className="w-full h-14 bg-slate-50/50 border border-border rounded-2xl px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-primary transition-all">
                    ...
                    <div className="space-y-3">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Issuing Node</Label>
                      <Input name="organization" value={form.organization} onChange={handleChange} className="h-14 bg-slate-50/50 border-border rounded-2xl font-bold" placeholder="E.g. Google Cloud" />
                    </div>

                    <div className="space-y-3">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Time Delta</Label>
                      <Input name="date" value={form.date} onChange={handleChange} className="h-14 bg-slate-50/50 border-border rounded-2xl font-bold" placeholder="E.g. 2024 - 2025" />
                    </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-64 bg-slate-50 border border-border rounded-[3rem] animate-pulse" />
          ))
        ) : (
          items.map((item, idx) => (
        ...
        {items.length === 0 && !loading && (
          <div className="col-span-full py-40 text-center bg-slate-50 rounded-[4rem] border border-border">
